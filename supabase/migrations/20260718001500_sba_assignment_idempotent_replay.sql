-- Permit a network retry only when it repeats the exact same submitted answer.
-- A different option remains blocked after the first atomic claim.

alter table public.sba_question_assignments
  add column if not exists selected_letter text
  check (selected_letter is null or selected_letter ~ '^[A-D]$');
...
drop function if exists public.claim_sba_question_assignment(uuid, text, text);
