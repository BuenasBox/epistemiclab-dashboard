...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: item } = await supabase.from('lab_items').select('reveal_content').eq('item_id', session.item_id).eq('lab_type', 'bottle').maybeSingle();
    ...
    if (session.state !== 'completed') {
      ...
      await recordLabEpistemicEvent(supabase, { userId: user.id, sessionId: session.id, occurredAt: now, sourceMode: 'bottle_lab_pro', eventId: `bottle:${session.id}:completed`, eventType: 'session_completed', payload: { item_id: session.item_id }, metadata: { lab_type: 'bottle' } });
    }
    return labJson({ ok: true, session_id: session.id, reveal: item.reveal_content });
  } catch { return labJson({ ok: false, error: 'Unable to reveal Bottle Lab' }, 500); }
});
