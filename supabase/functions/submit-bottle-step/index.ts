...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (body.step_kind === 'hypothesis') {
      await recordLabEpistemicEvent(supabase, { userId: user.id, sessionId: session.id, occurredAt: now, sourceMode: 'bottle_lab_pro', eventId: `bottle:${session.id}:${key}:decision`, eventType: 'decision_made', payload: { item_id: session.item_id, step_key: stepKey, result_band: evaluation.result.band }, evidence: { selected: evaluation.evidence.selected, ignored: evaluation.evidence.ignored }, metadata: { evaluation_version: item.evaluation_version, bias: evaluation.bias } });
      if (answer.confidence) await recordLabEpistemicEvent(supabase, { userId: user.id, sessionId: session.id, occurredAt: now, sourceMode: 'bottle_lab_pro', eventId: `bottle:${session.id}:${key}:confidence`, eventType: 'confidence_selected', payload: { confidence: evaluation.confidence.scale, calibration_band: evaluation.calibration.band }, evidence: { strength: evaluation.evidence.strength }, metadata: { evaluation_version: item.evaluation_version } });
