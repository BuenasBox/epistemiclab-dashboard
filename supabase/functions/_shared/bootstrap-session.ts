...
function roleFor(user: AnyRecord | null) {
  return user?.app_metadata?.role || user?.user_metadata?.role || 'authenticated';
}

function notificationSummary(notifications: AnyRecord | null) {
  const pending = Array.isArray(notifications?.pending) ? notifications.pending : [];
  const priorities = pending
    .map((item: AnyRecord) => Number(item.priority))
    .filter((priority: number) => Number.isFinite(priority));
...
export function projectBootstrapSession(readModel: AnyRecord, user: AnyRecord | null) {
