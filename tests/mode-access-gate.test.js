...
test('paid adaptive and open response modes request active enforcement before starting', () => {
  ...
  [adaptive, openResponse].forEach((html) => {
...
test('diagnostic modes remain public according to the matrix', () => {
  const diagnostic = fs.readFileSync(
    path.join(__dirname, '..', 'diagnostic-sba', 'index.html'),
    'utf8',
  );

  assert.doesNotMatch(diagnostic, /mode-access-gate\.js/);
});
