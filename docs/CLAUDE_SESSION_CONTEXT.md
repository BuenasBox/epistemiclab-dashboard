...
- **Fase 7 — Observabilidad:** ✅ completa. Vercel Web Analytics en las 17 páginas (commit `499ebc6`). Sentry integrado en las 17 páginas vía CDN bundle + CSP actualizado en `vercel.json` (commit `03b12ef`), con `defer` + guardia `DOMContentLoaded` en el init para evitar el `ReferenceError` de scripts inline no diferibles (commit `be11f0e`). ⏳ Pendiente manual de Erick: clic en "Enable" dentro de Vercel Dashboard > Analytics (no automatizable vía API/MCP).
...
- **Índices FK:** migración `20260718013917_add_assignment_fk_covering_indexes` (índices en `or_question_assignments.item_id` y `sba_question_assignments.question_id`) aplicada al proyecto remoto vía MCP y verificada: el advisor `unindexed_foreign_keys` quedó limpio. El archivo local se renombró de `20260718020000_...` a `20260718013917_...` para que su timestamp coincida exactamente con la versión que `apply_migration` registró en remoto (el MCP asigna el timestamp al momento de ejecución, no al del archivo) — evita que `supabase db push`/`migration list` vean el archivo como pendiente y lo reapliquen bajo una segunda versión.
